...
- 当前执行顺序：
  ...
  9. `done` 控制器拆分
  10. `done` 核心模型类型化
  11. `done` merge / parser / fallback 规则收口
  12. `done` 刷新调度边界收口
  13. `done` parser / merge / localization 单测
  14. `done` send runtime 决策矩阵单测
  15. `done` helper 状态变更回归测试
  16. `done` 受限环境预期测试
...
当前第一阶段 1-16 已完成，下一阶段重点不再是补大块基础设施，而是继续做剩余收口：

- 继续压缩 `CodexTaskmasterApp.swift` 中残留的展示与编排语义
- 继续统一 loop / send 相关 UI 编排模板
- 维持“每切片构建 + 启动验证 + 独立 commit”的推进方式

基础边界已经明显稳定，后续工作应尽量以小切片方式推进，避免重新引入控制器膨胀和协议分叉。
